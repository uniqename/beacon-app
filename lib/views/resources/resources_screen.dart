...
class _ResourcesScreenState extends State<ResourcesScreen> with TickerProviderStateMixin {
  ...
  Future<void> _loadResources() async {
    ...
    // Prepend admin-added shelters from local DB for the shelter tab
    List<Resource> extra = [];
    if (currentType == ResourceType.shelter) {
      try {
        final db = await LocalDatabaseService.database;
        final rows = await db.query('shelters', where: 'is_active = 1');
        extra = rows.map((r) => Resource(
          id: r['id'] as String,
          name: r['name'] as String,
          description: (r['services'] as String?) ?? '',
          type: ResourceType.shelter,
          status: ResourceStatus.available,
          address: '${(r['address'] as String?) ?? ''}, ${(r['city'] as String?) ?? ''}'.trim().replaceAll(RegExp(r'^,\s*|,\s*$'), ''),
          phone: r['phone'] as String?,
          services: ((r['services'] as String?) ?? '').split(',').map((s) => s.trim()).where((s) => s.isNotEmpty).toList(),
        ...
        developer.log('DB shelter load error: $e');
      }
    }

    setState(() {
      _allResources = [...extra, ...resources];
      _filteredResources = [...extra, ...resources];
      _isLoading = false;
    });
  }
