...
import 'helper_application_screen.dart';
    ...
    try {
      ...
      if (user == null && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Failed to create account. Please try again with a different email address or check that your password is at least 6 characters.'),
            backgroundColor: Colors.red,
            duration: Duration(seconds: 5),
          ),
        );
      } else if (user != null && mounted) {
        // Check if user is counselor or volunteer - redirect to application screen
        if (_selectedUserType == UserType.counselor || _selectedUserType == UserType.volunteer) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Account created! Please complete your ${_selectedUserType == UserType.counselor ? 'counselor' : 'volunteer'} application.'),
          ...
          if (mounted) {
            final result = await Navigator.of(context).pushReplacement(
              MaterialPageRoute(
                builder: (context) => HelperApplicationScreen(
                  userId: user.id,
                  userType: _selectedUserType == UserType.counselor ? 'counselor' : 'volunteer',
                ),
              ),
            );

            // After application submission, go to home screen
          ...
          // Regular survivor or admin - go straight to home
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
