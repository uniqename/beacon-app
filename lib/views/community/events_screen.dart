...
class _EventsScreenState extends State<EventsScreen>
  ...
  List<BeaconEvent> _displayEvents = List.from(_events);
  ...
  Future<void> _loadDbEvents() async {
    try {
      ...
      if (rows.isNotEmpty && mounted) {
        final dbEvents = rows.map((r) => BeaconEvent(
          id: r['id'] as String,
          ...
          capacity: 0,
          category: 'Beacon Event',
          color: const Color(0xFFF0562D),
        )).toList();
        setState(() => _displayEvents = [...dbEvents, ..._events]);
      }
    } catch (_) {}
  }
  ...
  List<BeaconEvent> get _rsvpdEvents =>
      _displayEvents.where((e) => _isRsvpd(e.id)).toList();
